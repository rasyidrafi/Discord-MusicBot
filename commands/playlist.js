...
module.exports = {
	info: {
		name: "playlist",
		description: "To play playlist songs :D",
...
async	function play(guild, song) {
  ...
  let afk = JSON.parse(fs.readFileSync("./afk.json", "utf8"));
    ...
    if (!song){
      if (!online.afk) {
        sendError(`Leaving the voice channel, because there is no songs in the queue. If you like the bot stay 24/7 in voice channel run **${client.config.prefix}afk**`, message.channel)
